...
def main(**kwargs):
    ...
    #if it doesn't start with oomp add it
    if not doc_name.startswith("oomp"):
        doc_name = "oomp_" + doc_name
    doc_name_undercore = doc_name.replace(" ", "_") 
    doc_name_underscore_no_oomp = doc_name_undercore.replace("oomp_", "")
    doc_name_space = doc_name.replace("_", " ")
    doc_folder = f"C:\\gh\\oomlout_oomp_part_generation_version_1\\parts\\{doc_name_underscore_no_oomp}"
    ...
    if True:
        ...
        #send doc name
        pyautogui.typewrite(doc_name_space)
        time.sleep(2)
